...
- Theme contrast gate: `pnpm theme:contrast` measures WCAG contrast (and colour-vision-deficiency separation) for the accessibility-first themes from `frontend/src/styles/tokens/colors.css`; `--all` reports every theme, `--markdown --cvd` prints PR-ready tables. Theme evidence screenshots: `pnpm theme:screenshots` (its own Playwright config) writes `screenshots/themes/*.png`; the same spec under `pnpm test` keeps its screenshots in the test output directory.
