...
- Lint: `pnpm lint`; Type-check: `pnpm typecheck` (runs per package). The root lint command is the single entry point for blocking Oxlint and Knip file/export/type/duplicate checks, residual ESLint, advisory anti-slop, and the remaining advisory Knip checks.
...
- Treat blocking lint as the new-code floor. Knip's file, export, type, and duplicate-export categories are blocking; advisory anti-slop and the remaining advisory Knip output record existing debt. Address relevant findings without broad suppressions. See `references/anti-slop.md` and `references/oxlint-overlap.md`.
