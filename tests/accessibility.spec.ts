...
test('Home and About are axe-clean and the modal contains and restores focus', async ({ page }) => {
  ...
  const themeTrigger = page.getByRole('button', { name: /\(sharp\)|\(rounded\)|OLED|Dusk|Forge|Ember|Aurora|Night Owl|Terracotta|Colorblind Safe|Low Fatigue|High Legibility/ }).last();
