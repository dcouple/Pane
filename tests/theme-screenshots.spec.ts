...
const combinedDiff = {
  diff: [
    ...
    'diff --git a/frontend/src/utils/terminalTheme.ts b/frontend/src/utils/terminalTheme.ts',
    'index 3333333..4444444 100644',
    '--- a/frontend/src/utils/terminalTheme.ts',
    '+++ b/frontend/src/utils/terminalTheme.ts',
    '@@ -70,9 +70,17 @@ export const getTerminalTheme = (): ITheme => {',
    '-  const selectionBg = getComputedStyle(document.documentElement)',
    "-    .getPropertyValue('--color-terminal-selection-bg')",
    '-    .trim();',
    '+  // Only set when the theme defines it — dark themes keep the xterm default.',
    "+  const selectionBg = readTerminalVariable('--color-terminal-selection-bg');",
    '   const theme: ITheme = {',
  ...
  stats: { additions: 14, deletions: 3, filesChanged: 2 },
  changedFiles: ['frontend/src/contexts/ThemeProvider.tsx', 'frontend/src/utils/terminalTheme.ts'],
};
